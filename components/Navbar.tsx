...
import { Menu, X, Search, Home, User, Layers, Phone } from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="w-full px-6  py-4 flex items-center fixed justify-between bg-white z-50">
       <Link href="/">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                href={item.href}
                key={item.name}
                className="relative px-8 py-4 flex items-center gap-2 cursor-pointer"
