    ...
    const menuItems = [
      ...
      { name: "Services", href: "/services", icon: <Layers size={18} /> },
      {
        ...
        children: [
          ...
          // isFile: true means use standard <a> tag for PDF/Download
          {
            name: "Brochure",
            href: "/brouchure.pdf",
                      ...
                      {isHover && (
                        ...
                        >
                          {item.children?.map((child) => {
                            // ✅ LOGIC: If it's a file (PDF), use <a>, otherwise use <Link>
                            if (child.isFile) {
                              return (
                                <a
                                  key={child.name}
        ...
        >
          ...
          <div className="flex flex-col gap-4">
            {menuItems.map((item) => {
              ...
              // ✅ CHANGED: <a> to <Link> for mobile top-level items
              return (
                <Link
                  href={item.href}
                  key={item.name}
