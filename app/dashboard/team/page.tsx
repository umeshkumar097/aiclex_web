  ...
  return (
    <div className="animate-fade-in p-6 bg-white rounded-3xl shadow-sm border border-gray-100">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-black text-[#001341] capitalize">team Management</h2>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              ...
              <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">name</th><th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider">role</th>
            ...
            {data.length === 0 ? (
              ...
              data.map((item: any) => (
                <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="p-4 text-sm text-gray-600 truncate max-w-[150px]">{String(item.name || "-")}</td><td className="p-4 text-sm text-gray-600 truncate max-w-[150px]">{String(item.role || "-")}</td>
